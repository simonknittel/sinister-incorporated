...
const schema = z.object({
  ...
  collectionEndedAt: z.preprocess((arg) => {
    if (!arg || !(typeof arg === "string")) return null;
    const date = new Date(arg.trim());
...
export const createProfitDistributionCycle = createAuthenticatedAction(
  ...
  async (formData, authentication, data, t) => {
     ...
     */
    if (data.collectionEndedAt && data.collectionEndedAt <= new Date())
      return {
     ...
     * Create
     */
    const collectionEndedAt = data.collectionEndedAt
      ? new Date(data.collectionEndedAt)
      : null;
    if (collectionEndedAt) collectionEndedAt.setHours(0, 0, 0, 0);
    ...
    revalidatePath(`/app/silc/profit-distribution/${created.id}`);
